...
//	마지막에 추가하는 정보이므로 순위는 전달받지 않는다.
	private boolean append(BoxOfficeVO boxOfficeVO) throws IOException{
		BufferedWriter bufferedWriter = DBConnecter.getAppend();		//DBConnecter에서 메모장과 연결해줬다
...
//			기존 정보 중 순위를 제외한 5개 정보
			String excludedRankingOrigin = line.substring(line.indexOf("\t"));	// tab을 기준으로 그다음것부터 가져옴(substring)
...
//		byte[]을 String생성자에 전달하여 디코딩(문자열로 바꾸기)한다.  = 해독하기
...
//		줄바꿈 문자 유무 검사의 결과를 새롭게 추가할 정보 앞에 연결해준다.
		bufferedWriter.write(newLine + boxOfficeVO.toString());		//write()메소드는 toString을 직접써줘야함
//		추가해줬으면 닫아주기
...
//				해당 순위에 새로운 정보를 삽입해준다.
				temp += boxOfficeVO + "\n";
//				삽입한 이후부터 원래순위를 다시 담아주기 위한 기준을 FLAG(check)로 선언해주었다.
//				FLAG는 true로 변경해준다.
				check = true;
			}
//			FLAG가 true라면 삽입 된 후이기 때문에, 새로운 순위로 수정해주어야 한다.
//			삽입할 순위 + 1 부터 1씩 증가시키면 모두 1순위씩 밀려나게 된다.
			temp += check ? ++newRanking + line.substring(line.indexOf("\t")) : line;	//전위형! 1~
			temp += "\n";	//마지막 삽입해줬으면 줄바꿈해주고 더이상 읽지못하도록 한다.
		}
//		삽입해줬으면 닫아주기
		bufferedReader.close();
...
//		기존 정보 중 순위를 비교할건데
		while((line = bufferedReader.readLine()) != null) {
//			tab을 기준(split)으로 0번째 방에 있는 문자열을 Integer로 boxing하여 
//			입력순위와 비교한다.
//			기존 순위: Integer.valueOf(line.split("\t")[0])
			if(Integer.valueOf(line.split("\t")[0]) == boxOfficeVO.getRanking()) {
//				순위가 같다면 수정 대상이기 때문에 기존 정보 대신 새롭게 수정된 정보를 temp에 담아준다.
				temp += boxOfficeVO + "\n";
//				FLAG는 true로 변경한다.
				check = true;
...
//	외부에서 삭제할 순위를 전달받는다.
	public boolean delete(int ranking) throws IOException{
		...
		BufferedWriter bufferedWriter = null;
		String line = null, temp = "";
		int deletedRanking = ranking;		//삭제될 랭킹
		boolean check = false;
