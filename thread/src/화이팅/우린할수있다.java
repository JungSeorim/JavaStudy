...
//      기존의 값을 길이로 변경 후 lengths에 저장 
      datas.stream().map(data -> data.length()).forEach(lengths::add);		//배열에 담긴 값들이 람다식의 변수(data)로 쓰이는 것
...
//      전달받은 값이 0밖에 없으면 size()는 0이므로 0 리턴
   ...
   public static void main(String[] args) {
      ...
      a.stream().forEach(b->System.out.println(b+1));	//b에 들어가는 값은 배열에 담긴 값이라고 보면된다.
   }
}