...
//      USER_ID 컬럼에서 사용자가 입력한 id 검사
      String query = "SELECT COUNT(USER_ID) FROM TBL_USER WHERE USER_ID = ?"; // ? : 변수가 들어갈 자리
      boolean check = false;
      try {
//         연결 객체 가져오기
...
//         1이라면 사용자가 입력한 아이디가 1개 조회된 것이기 때문에 중복된 아이디이다.
         check = resultSet.getInt(1) == 1;		// 1은 true ?
...
//   회원가입
   public void insert(UserVO userVO) {
      ...
      try {
         ...
         preparedStatement.setString(10, userVO.getUserRecommanderId());
...
//   회원정보 수정
   public void update(UserVO userVO) {
      ...
      try {
         ...
         preparedStatement.setString(8, userVO.getUserRecommanderId());
...
//   회원정보 조회
   public UserVO selectUser(int userNumber) {
      ...
      try {
         ...
         userVO.setUserRecommanderId(resultSet.getString(++i));
